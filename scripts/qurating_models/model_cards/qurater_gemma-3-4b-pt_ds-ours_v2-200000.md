...
# QuRater Core Educational Scorer (Gemma 3 4B)

Model repository: `AI-for-Education/qurater_gemma-3-4b-pt_ds-ours_v2-200000`

Backbone: Gemma-3-4B-PT

This model is a legacy-named QuRater sequence-classification scorer for educational text. It takes a text passage as input and outputs one scalar logit for each criterion in the core educational-quality rubric. It is intended for ranking, filtering, corpus curation, and reward-modeling experiments, not for text generation.

This is the core Gemma QuRater used in the web-scale FineWeb-Edu-Fortified scoring and filtering pipeline. The newer naming convention for this model family is `edu-qurating-core-ed-*`; this card keeps the exact legacy repository id for compatibility with existing artifacts.
