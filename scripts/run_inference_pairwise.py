...
class TokenizeAndChunk:
    ...
    def __init__(self, tokenizer_name: str, text_field: str = "text", tokens: int = 512):
        self.tokens = tokens
...
class ModelAnnotator:
    ...
    def score_chunks(self, chunks_token_ids: List[torch.Tensor], chunks_token_counts: torch.Tensor) -> torch.Tensor:
...
def main():
    parser = argparse.ArgumentParser(description="Run inference on pairwise dataset texts")
    ...
    parser.add_argument("-t", "--tokens", type=int, default=512, help="Chunk size in tokens")
    ...
    annotator = ModelAnnotator(args.model, args.labels, args.batch_size)
    ...
    final_df = pd.concat([texts_df.reset_index(drop=True), results_df.reset_index(drop=True)], axis=1)
    ...
    for label in args.labels:
        if f"{label}_average" in final_df.columns:
            ...
            print(f"{label}: mean={scores.mean():.3f}, std={scores.std():.3f}, min={scores.min():.3f}, max={scores.max():.3f}")
