...
from transformers import AutoTokenizer, AutoModelForSequenceClassification, AutoConfig, HfArgumentParser, set_seed
import torch

from qurating.constants import RESULTS_DIR

from qurating.training import (
    PreferenceTrainer, TrainingArguments, DataCollator,
    confidence_mask, bce_with_temperature, LabelFilter, ConfidenceFilter
)
...
def validate_dataset_file(file_path):
    ...
    if not os.path.exists(file_path):
        ...
        logger.info("Please ensure the dataset has been generated using test_dataset_og.py")
        return False
    try:
...
def setup_model_and_tokenizer(args, label_names):
    ...
    model = create_model(args.model_name, config=config)
...
def prepare_datasets(dataset, script_args, training_args, label_names):
    ...
    dataset_filtered = dataset.filter(LabelFilter(label_names), num_proc=1, keep_in_memory=True)
    ...
    if training_args.confidence_threshold > 0.0:
        dataset_filtered = dataset_filtered.filter(
            ...
            num_proc=1, keep_in_memory=True
        )
    ...
    if len(dataset_filtered) > 10:  # Only split if we have enough data
        splits = dataset_filtered.train_test_split(test_size=script_args.eval_split_size, seed=42)
...
def run_training(model, tokenizer, train_dataset, eval_datasets, training_args, data_collator):
...
def main():
    ...
    try:
        ...
        script_args, training_args, data_collator = setup_training_args_and_collator(args, label_names, tokenizer)
        
        # Prepare datasets
        train_dataset, eval_datasets = prepare_datasets(dataset, script_args, training_args, label_names)
        
        # Run training
        success = run_training(model, tokenizer, train_dataset, eval_datasets, training_args, data_collator)
