...
# %%
annotator, tokenizer = init_annotator()
...
for batchi, batch_ds in enumerate(ds.batch(batch_size=outer_batch_size)):
    ...
    out_cols = ["id", *[col for col in results.column_names if col.endswith("_average")]]
    out_ds = results.select_columns(out_cols).add_column("dump", [subset]*len(results))
    out_ds.to_parquet(f"{subset}_{batchi :04d}.parquet")
